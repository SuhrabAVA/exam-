# Krisha App — объявления о недвижимости

Backend — Django (только REST API, отдаёт JSON), frontend — статический
...
# 4) создать базу krisha_db и загрузить schema.sql + seed.sql (psql не нужен)
...
- Параметры подключения — переменные окружения `DB_NAME`, `DB_USER`,
  `DB_PASSWORD`, `DB_HOST`, `DB_PORT` (по умолчанию `krisha_db` / postgres /
...
- `static/images/` — фото объявлений и аватары из задания
