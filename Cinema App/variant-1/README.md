# Cinema App — покупка билетов в кино
...
# 4) создать базу cinema_db и загрузить schema.sql + seed.sql (psql не нужен)
...
- Параметры подключения — переменные окружения `DB_NAME`, `DB_USER`,
  `DB_PASSWORD`, `DB_HOST`, `DB_PORT` (по умолчанию `cinema_db` / postgres /
...
- `static/images/` — постеры из задания
