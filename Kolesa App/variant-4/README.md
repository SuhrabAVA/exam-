# Kolesa App — доска объявлений авто
...
# 4) создать базу kolesa_db и загрузить schema.sql + seed.sql (psql не нужен)
...
- Параметры подключения — переменные окружения `DB_NAME`, `DB_USER`,
  `DB_PASSWORD`, `DB_HOST`, `DB_PORT` (по умолчанию `kolesa_db` / postgres /
...
- `static/images/` — фотографии авто и аватары из задания
