# Hotel Booking — бронирование номеров отеля
...
# 4) создать базу hotel_db и загрузить schema.sql + seed.sql (psql не нужен)
...
- Параметры подключения — переменные окружения `DB_NAME`, `DB_USER`,
  `DB_PASSWORD`, `DB_HOST`, `DB_PORT` (по умолчанию `hotel_db` / postgres /
...
- `static/images/` — фотографии номеров из задания
